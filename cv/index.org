...
#+begin_export latex
\begin{quote}
``I am a Mechatronics Engineer with great passion for technology. Along the time I decided to focus in Web Development, playing with JavaScript/TypeScript in full-stack development. Later I got in touch with the functional paradigm using F# (.NET platform) and now I see the static types as one of the most useful tools of a programming language. With the type system we can make wrong state unrepresentable.''  
\end{quote}
#+end_export
...
Full-stack Web Developer working in a consultancy project dealing with F#,
